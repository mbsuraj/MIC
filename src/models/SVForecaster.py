...
class SVForecaster(Forecaster):
    ...
    def fit(self, params):
        """
        Fit the Support Vector model with given parameters.
        """
    ...
    def search_and_fit(self):
        ...
        self.fitted_values = self.model.predict(X)
        self.save_search_results('SVM')
        print("Model fitted successfully with optimal parameters.")

    def save_model(self, path=None):
        """
        Save the fitted Support Vector model to a file using pickle.
    ...
    def load_model(self):
        """
        Load a previously saved Support Vector model from a file using pickle.
        """
        try:
    ...
    def output(self):
        ...
        Prints the model's details and, if the model has been fitted,
    ...
    def forecast(self, steps):
        """
        Forecast future values using the trained Support Vector model.
...
# Example usage:
# sv_forecaster = SVForecaster(data=data_series, lags=12)
# sv_forecaster.fit()
# sv_forecaster.save_model('sv_model.pkl')
# sv_forecaster.plot_fit_vs_actual(steps=10)

# To load the model later:
# sv_forecaster.load_model('sv_model.pkl')
# forecasted_values = sv_forecaster.forecast(steps=10)
# sv_forecaster.plot_fit_vs_actual(steps=10)
